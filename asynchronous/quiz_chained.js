...
import { buyTollRoadCard, topUpBalance, useTollRoad } from './quiz_chained_utils.js';
// const { buyTollRoadCard, topUpBalance, useTollRoad } = require('./quiz_chained_utils.js');

async function getTollAccess() {
    try {
        const card = await buyTollRoadCard(25);
        const topUp = await topUpBalance(card, 10);
        await useTollRoad(topUp);
        return;
    } catch (error) {
